...
use AppBundle\Form\NouveauMDPType;
 ...
 /**
   ...
   */
    ...
    public function changementMdp($mail,$token,Request $request)
    {
        $em=$this->getDoctrine()->getManager();
         $repo=$em->getrepository("AppBundle:User");
          $data=$repo->findby(array('email'=>$mail,'token'=>$token));
         
//          foreach ($data as $unUser)
//         {
//                    $user=$unUser;
//
//             if($unUser->getToken()!=$token)
//             {
//                $error = $request->attributes->get(
//                  Security::AUTHENTICATION_ERROR  );
//               return $this->render(path('login_check',array('ok'=>0)));
//             }
//         }
           $regist=new NouveauMDPType();
           $form=$this->createform($regist,$user);
         $form->handleRequest($request);      
           if($form->isSubmitted() && $form->isValid() )
        {
        ...
        return $this->render('user/ModifMdp.html.twig',array('formulaire'=>$form->createView() ));
    }
